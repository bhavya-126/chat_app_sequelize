...
let getMulterMiddleware = (formData) => (req, res, next) => {
    ...
    if (formData.file && Object.keys(formData.file).length) {
        ...
        return uploadMiddleware.single(formData.file[fileField])(req, res, (err) => {
            if (err) {
                // return helpers.createErrorResponse(err.message, ERROR_TYPES.BAD_REQUEST);
                res.status(400).json(helpers.createErrorResponse(err.message, ERROR_TYPES.BAD_REQUEST))
            } else {
                next()
            }
    ...
    return (request, response) => {
        ...
        handler(payload)
            .then((result) => {
                ...
                if (result.video) {
                    response.writeHead(206, result.headers);
                    result.video.pipe(response);
                    return;
                }
                if (result.filePath) {
